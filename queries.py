...
def save_prediction_session(db: Session, uid: str, original_path: str, predicted_path: str, username: str):
...
def get_predictions_by_score(db: Session, min_score: float, username: str):
    rows = (
        db.query(PredictionSession.uid, PredictionSession.timestamp, DetectionObject.score)
        .join(DetectionObject, PredictionSession.uid == DetectionObject.prediction_uid)
        .filter(
...
def count_recent_predictions(db: Session, username: str, since: datetime) -> int:
    return (
        db.query(PredictionSession)
        .filter(PredictionSession.timestamp >= since, PredictionSession.username == username)
        .count()
    )

def get_detection_objects_for_recent_predictions(db: Session, username: str, since: datetime):
    subquery = (
        db.query(PredictionSession.uid)
        .filter(PredictionSession.timestamp >= since, PredictionSession.username == username)
        .subquery()
